...
def Get3ptCorr(icfg,ism,tsink,Projector,DS,ND,iPoF=0):
    ...
    return (cf3ptdir+thisREvecDir+CreateCfg(icfg,DelLime=True)+'_k'+str(kud)+'_tsrc'+str(iPoF)+'sm'+str(ism)+'_'+REvecFlag+'PoF'+str(PoFShifts)+'D'+str(PoFDelta)+
...
def Get3ptCorrjsm(icfg,ism,jsm,tsink,Projector,DS,ND,iPoF=0):
    ...
    return (cf3ptdir+thisCMDir+CreateCfg(icfg,DelLime=True)+'_k'+str(kud)+'_tsrc'+str(iPoF)+'sm'+str(ism)+'_si'+str(jsm)+'GMA'+str(Projector)+
